package cn.redcdn.hnyd.profiles.voicedetection;
...
//				}
                ...
                {
                    return new TAmp(time*step, peak.amplitude);
                }
            }
            time++;
        }
    ...
    {
        AAmp[] aaque = new AAmp[40]; //分析640ms的数据
...
//				aaque[time] = new parseAudioDelay.AAmp(time*step, peak);
                aaque[time] = new AAmp(time*step, peaks);
            }
            time++;
        }
        ...
        double min2812e = 0.0;
        double max2812e = 0.0;
        AAmp2[] a2812aque = new AAmp2[10]; //2812.5能量前十
        int ai = 0;
        double mine = 0.0;
        double maxe = 0.0;
        AAmp2[] axaque = new AAmp2[10]; //所有频段能量前十
    ...
    //按时间排序
    private static void sequeByT(AAmp2[] que)
    {
