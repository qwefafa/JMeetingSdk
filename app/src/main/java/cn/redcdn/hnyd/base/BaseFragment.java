...
import cn.redcdn.hnyd.im.activity.GroupAddActivity;
import cn.redcdn.hnyd.im.bean.GroupMemberBean;
import cn.redcdn.hnyd.im.dao.GroupDao;
import cn.redcdn.hnyd.officialaccounts.activity.ArticlePreviewActivity;
import cn.redcdn.hnyd.profiles.activity.OutDateActivity;
import cn.redcdn.hnyd.util.CustomToast;
import cn.redcdn.hnyd.util.TitleBar;
import cn.redcdn.log.CustomLog;

import static android.content.ContentValues.TAG;
import static cn.redcdn.hnyd.officialaccounts.activity.ArticlePreviewActivity.INTENT_DATA_ARTICLE_ID;
import static cn.redcdn.hnyd.profiles.ProfilesFragment.ARTICLE_PREVIEW;
import static cn.redcdn.hnyd.profiles.ProfilesFragment.GROUP_TYPE;
import static cn.redcdn.hnyd.profiles.ProfilesFragment.PERSON_TYPE;
import static cn.redcdn.hnyd.profiles.ProfilesFragment.WE_TYPE;
...
//                        if (https.contains(HTTPS)) {
                if (split2[0].equals(PERSON_TYPE)) {
                    ...
                    if (days >= 7) {
                        ...
                        loginUserInfo = AccountManager.getInstance(getActivity()).getmAuthInfo();
                        ;
                        memberDateList = mGroupDao.queryGroupMembers(mGroupId);
                        if (memberDateList.containsKey(loginUserInfo.getNubeNumber())) {
...
//                    }
                ...
                Toast.makeText(getActivity(), getString(R.string.parseTheQrcodeFailed), Toast.LENGTH_LONG).show();
    ...
    public void showLoadingView(String message,
        ...
        dialog = cn.redcdn.hnyd.util.CommonUtil.createLoadingDialog(getActivity(), message,
                listener);
