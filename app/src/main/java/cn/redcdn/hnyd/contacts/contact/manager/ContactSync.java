...
import cn.redcdn.hnyd.contacts.contact.StringHelper;
import cn.redcdn.hnyd.contacts.contact.butelDataAdapter.ContactSetImp;
import cn.redcdn.hnyd.contacts.contact.database.DBConf;
...
public class ContactSync extends Thread {
    ...
    private List<ContanctsInfo> totalList = null;
                ...
                try {
                    if (state != STOP_STATE) {
                        if (list.get(i) != null && list.get(i).size() > 0) {
                            ...
                            List<Users> users = search.searchAccountSync(
                                     list.get(i).toArray(array));

                            if (users != null) {
                                write(KeyEventConfig.SYNC_CONTACTS_INFO_MOBILE
                                                + "_ok"
                                                + "_"
                                                + AccountManager.getInstance(context).getmAuthInfo().getNubeNumber());
                            } else {
                                write(KeyEventConfig.SYNC_CONTACTS_INFO_MOBILE
                                                + "_fail"
                                                + "_"
...
//                                }
    ...
    private void compareContacts(List<Users> list,
            ...
            for (Users entry : list) {
                try {
                    ...
                    if (c != null && entry != null) {
                        ...
                        if(entry.nickName!=null&&c.getNickname()!= null
                                &&!entry.nickName.equals(c.getNickname())) {
                            c.setNickname(entry.nickName);
                        }

                        if(entry.mail!=null&&c.getEmail()!=null
                                &&!entry.mail.equals(c.getEmail())){
                            c.setEmail(entry.mail);
                        }

                        if(entry.mobile!=null&&c.getNumber()!=null
                                &&!entry.mobile.equals(c.getNumber())){
                            c.setNumber(entry.mobile);
                        }

                        if(entry.headUrl!=null&&c.getHeadUrl()!=null
                                &&!entry.headUrl.equals(c.getHeadUrl())){
                            c.setHeadUrl(entry.headUrl);
                        }
