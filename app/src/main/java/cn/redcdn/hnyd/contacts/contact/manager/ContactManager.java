...
import cn.redcdn.hnyd.contacts.contact.interfaces.ContactOperation;
...
public class ContactManager implements ContactOperation {
    ...
    private void searchUser(final String nubeNumber, final ContactCallback callback){
        ...
        SearchAccount searchUser = new SearchAccount(){
            @Override
            protected void onSuccess(List<Users> responseContent) {
                ...
                if (list != null && list.size() > 0) {
                    ...
                    friendInfo.setNubeNumber(list.get(0).nubeNumber);
                    friendInfo.setName(list.get(0).nickName);
                    friendInfo.setHeadUrl(list.get(0).headUrl);
                    friendInfo.setEmail(list.get(0).mail);
                    ...
                    friendInfo.setNumber(list.get(0).mobile);
                    handleAddContact(friendInfo,callback);
                }else{
        ...
        int result = searchUser.searchAccount(arraylist);
