...
public class RecommendManager {
  ...
  private SearchAccount searchUsers;
  ...
  private void newDoSearchAccountByMobile(final int searchType,List<String> list,final String fromWhere){
    ...
    searchUsers.searchAccount(list.toArray(array));
  }

  private List<Contact> NewChangeBeans(List<Users> responseContent) {
    List<Contact> ccs = new ArrayList<Contact>();
    if (responseContent != null && !responseContent.isEmpty()) {
      Contact c = null;
      for (Users u : responseContent) {
        c = new Contact();
        c.setNubeNumber(u.nubeNumber);
        c.setAppType("mobile");
        c.setNickname(u.nickName);
        c.setName(u.nickName);
        c.setPicUrl(u.headUrl);
        c.setHeadUrl(u.headUrl);
        c.setContactUserId(u.uid);
        c.setContactId(CommonUtil.getUUID());
        c.setUserType(1);
        c.setUserFrom(1);

        if(null!=u.mobile&&!u.mobile.isEmpty()){//手机号
          c.setNumber(u.mobile);
        }else if(null!=u.mail&&!u.mail.isEmpty()){//邮箱号
          c.setEmail(u.mail);
        }
        ccs.add(c);
      }
    ...
    try {
      ...
      if (localCursor != null && localCursor.getCount() > 0) {
        ...
        while (localCursor.moveToNext()) {
          ...
          // 去自己
          if (AccountManager.getInstance(mContext).getmAuthInfo() != null
              && number.equals(AccountManager.getInstance(mContext)
                  .getmAuthInfo().getMobile())) {
            continue;
          }
