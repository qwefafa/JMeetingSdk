package cn.redcdn.hnyd.im.adapter;
...
import cn.redcdn.hnyd.im.activity.EmbedWebViewActivity;
import cn.redcdn.hnyd.im.activity.ViewPhotosActivity;
import cn.redcdn.hnyd.im.bean.BookMeetingExInfo;
import cn.redcdn.hnyd.im.bean.ButelFileInfo;
...
import cn.redcdn.hnyd.im.bean.PhotoBean;
import cn.redcdn.hnyd.im.bean.ShowNameUtil;
import cn.redcdn.hnyd.im.bean.ShowNameUtil.NameElement;
import cn.redcdn.hnyd.im.bean.WebpageBean;
import cn.redcdn.hnyd.im.collection.CollectionFileManager;
import cn.redcdn.hnyd.im.column.NoticesTable;
import cn.redcdn.hnyd.im.dao.GroupDao;
import cn.redcdn.hnyd.im.dao.NoticesDao;
import cn.redcdn.hnyd.im.fileTask.ChangeUIInterface;
import cn.redcdn.hnyd.im.fileTask.FileTaskManager;
import cn.redcdn.hnyd.im.manager.CollectionManager;
import cn.redcdn.hnyd.im.manager.HtmlParseManager;
import cn.redcdn.hnyd.im.manager.HtmlParseManager.OnClickBack;
import cn.redcdn.hnyd.im.util.IMCommonUtil;
import cn.redcdn.hnyd.im.util.MediaFile;
import cn.redcdn.hnyd.im.util.PlayerManager;
import cn.redcdn.hnyd.im.util.smileUtil.EmojiconTextView;
import cn.redcdn.hnyd.im.view.BottomMenuWindow.MenuClickedListener;
import cn.redcdn.hnyd.im.view.MedicalAlertDialog;
import cn.redcdn.hnyd.im.view.SharePressableImageView;
import cn.redcdn.hnyd.im.view.XCRoundImageViewByXfermode;
...
import cn.redcdn.hnyd.profiles.activity.ChatCollectionActivity;
...
import static cn.redcdn.hnyd.util.CommonUtil.getString;
...
//        initReceive();
    ...
    private void handleDescription(final NoticesBean bean,
        ...
        if (text.contains(groupDao.getString(R.string.send_friend_validation))) {
            ...
            spanableInfo.setSpan(new Clickable(clickListener), text.length() - 6, text.length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
    ...
    private void handleChatRecordMsg(final NoticesBean bean, final ViewHolder holder) {
        ...
        try {
            ...
            holder.chatRecordDetail.setText(detailContentStr.substring(0, detailContentStr.length() - 1));
        } catch (Exception e) {
            LogUtil.e("JSONArray Exception", e);
        }
...
//                holder.chatRecordTitleTxt.setText(bean.getTitle());
//            }else{
//                JSONObject titleObj = new JSONObject(bean.getTitle());
//                holder.chatRecordTitleTxt.setText(titleObj.optString("msgInfo"));
//            }
//
...
//                                        bean.getStatus(), null);
                                ...
                                callbackIf.onMsgForward(bean.getId(),
                                        bean.getSender(), bean.getType(),
                                        bean.getStatus(), null, holder.chatRecordTitleTxt.getText().toString(), null, null, null, null, null, null, null);
                            }
                        }
                            ...
                            try {
                                ...
                                if (bodyArray != null && bodyArray.length() > 0) {
                                    ...
                                    for (int i=0;i<detailArray.length();i++){
                                        ...
                                        }else if(tmpObj.optInt("type") == FileTaskManager.NOTICE_TYPE_ARTICAL_SEND){
                    ...
                    menuDlg.addButtonFirst(new MenuClickedListener() {
                        ...
                        public void onMenuClicked() {
                            ...
                            if (callbackIf != null) {
                                ...
                                callbackIf.onMsgForward(bean.getId(),
                                        bean.getSender(), bean.getType(),
                                        bean.getStatus(), null, holder.articleTitle.getText().toString(), null, null, null, null, null, null, null);
    ...
    private void handleInviteMeeting(final NoticesBean bean,
        ...
        holder.vcardNameTxt.setText(showName + getString(R.string.creat_vedio_consultation) + meetingRoom);
        ...
        holder.vcardBg.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View arg0) {
                ...
                int isSuccess = meetingManager.joinMeeting(mMeetingNum,
                        new MedicalMeetingManage.OnJoinMeetingListener() {
                            ...
                            public void onJoinMeeting(String valueDes, int valueCode) {
                                if (valueCode < 0) {
                                    CustomToast.show(mContext, mContext.getString(R.string.join_consultation_fail_sigh), 1);
                                }
                            }
                        });
                ...
                if (isSuccess == 0) {
                    ...
                    CustomToast.show(mContext, getString(R.string.join_consultation_fail_sigh), 1);
                }
            }
                    ...
                    menuDlg.addButtonThird(new MenuClickedListener() {
                        ...
                        public void onMenuClicked() {
                            ...
                            if (callbackIf != null) {
                                callbackIf.onMsgForward(bean.getId(),
                                        bean.getSender(), bean.getType(),
                                        bean.getStatus(), null, null, null, null, showName2, meetingRoom2, null, null, null);
                            }
                        }
                    }, mContext.getString(R.string.chat_forward));
                    menuDlg.addButtonForth(new MenuClickedListener() {
    ...
    private void handleBookMeeting(final NoticesBean bean,
        ...
        String meetingDetail =
                mContext.getString(R.string.theme) + exInfo.getMeetingTheme()
                        + "\n"
                        + mContext.getString(R.string.meeting_time) + DateUtil.formatMs2String(exInfo.getMeetingTime(),
                    ...
                    menuDlg.addButtonThird(new MenuClickedListener() {
                        ...
                        public void onMenuClicked() {
                            ...
                            if (callbackIf != null) {
                                callbackIf.onMsgForward(bean.getId(),
                                        bean.getSender(), bean.getType(),
                                        bean.getStatus(), null, null, null, null, exInfo.getBookName(), exInfo.getMeetingRoom(), exInfo.getMeetingTheme(), DateUtil.formatMs2String(exInfo.getMeetingTime(),
                            ...
                            holder.noticeTxt, new OnClickBack() {
                                ...
                                public void OnClick(String mUrl) {
                                    ...
                                    if (mUrl.contains(MedicalMeetingManage.JMEETING_INVITE_URL)) {
                                        String meetingId = mUrl.substring(MedicalMeetingManage.JMEETING_INVITE_URL.length());
                                ...
                                linkView.setOnClickListener(new OnClickListener() {
                                    ...
                                    public void onClick(View v) {
                                        ...
                                        if (srcUrl.contains(MedicalMeetingManage.JMEETING_INVITE_URL)) {
                                            String meetingId = srcUrl.substring(MedicalMeetingManage.JMEETING_INVITE_URL.length());
                                            meetingLinkClickListener.meetingLinkClick(meetingId);
                                        } else {
                                            Intent intent = new Intent(mContext,
                                                    EmbedWebViewActivity.class);
...
//        }
        ...
        RelativeLayout.LayoutParams lp = (RelativeLayout.LayoutParams) holder.imgLayout
                .getLayoutParams();
//        int width =  (int) mContext.getResources().getDimension(R.dimen.x157);
//        int height = (int) mContext.getResources().getDimension(R.dimen.y280);
                        ...
                        public void onMenuClicked() {
                            ...
                            if (checkDataValid(bean.getType(), bean.getBody())) {
                                ...
                                if(bean.getType() == FileTaskManager.NOTICE_TYPE_VEDIO_SEND){
                                    if(fileInfo.getRemoteUrl().toLowerCase().endsWith(".mp4")){
                                        picPath = fileInfo.getLocalPath().length() > 0 ?
                                                fileInfo.getLocalPath() : fileInfo.getThumbUrl();
                                    }else{
                                        picPath = fileInfo.getThumbUrl();
                                    }
                                }else{
                                    picPath = fileInfo.getLocalPath().length() > 0 ?
                                            fileInfo.getLocalPath() : fileInfo.getThumbUrl();
...
//                Intent i = new Intent(mContext, ScanPhotosAndVideosActivity.class);
//                i.putExtra(ScanPhotosAndVideosActivity.KEY_THREAD_ID, bean.getThreadsId());
//                i.putExtra(ScanPhotosAndVideosActivity.KEY_CURR_MSGID,bean.getId());
//                mContext.startActivity(i);
...
//        }
    ...
    private void handleVcardMessage(final NoticesBean bean,
        ...
        NameElement element = ShowNameUtil.getNameElement("", holder.nickName,
                    ...
                    menuDlg.addButtonFirst(new MenuClickedListener() {
                        ...
                        public void onMenuClicked() {
                            ...
                            if (callbackIf != null) {
                                callbackIf.onMsgForward(bean.getId(),
                                        bean.getSender(), bean.getType(),
                                        bean.getStatus(), null, null, holder.nickName, holder.nubeNumber, null, null, null, null, null);
                            }
                        }
                    }, mContext.getString(R.string.chat_forward));
                }
...
//        }
    ...
    private void showImage(boolean isSend, boolean isVideo,
        ...
        if(isVideo){
            ...
            }else if(isValidFilePath(localPath)){
...
//                            }
                            if (bean.getType() == FileTaskManager.NOTICE_TYPE_TXT_SEND) {
                                try {
                                    JSONArray bodyArray = new JSONArray(bean.getBody());
                                    if (bodyArray != null && bodyArray.length() > 0) {
                                        JSONObject bodyObj = bodyArray.optJSONObject(0);
                                        String oriText = bodyObj.optString("txt");
                                        String tmpStr = getNewTextMsg(oriText);
                                        bodyObj.put("txt", tmpStr);
                                        bean.setBody(bodyArray.toString());
                                    }
                    ...
                    menuDlg.addButtonFirst(new MenuClickedListener() {
                        ...
                        public void onMenuClicked() {
                            if (callbackIf != null) {
                                callbackIf.onMsgForward(bean.getId(),
                                        bean.getSender(), bean.getType(),
                                        bean.getStatus(), null, holder.noticeTxt.getText().toString(), null, null, null, null, null, null, null);
//                                if (v.getId() == holder.noticeTxt.getId()) {
//                                    callbackIf.onMsgForward(bean.getId(),
//                                            bean.getSender(), bean.getType(),
//                                            bean.getStatus(), null, holder.noticeTxt.getText().toString(), null, null, null, null, null, null, null);
//                                } else {
