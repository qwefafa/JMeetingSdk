...
import cn.redcdn.hnyd.im.dao.FriendsRelationDao;
import cn.redcdn.hnyd.im.dao.StrangerMsgDao;
import cn.redcdn.hnyd.im.interfaces.FriendCallback;
import cn.redcdn.hnyd.im.interfaces.IFriendRelation;
import cn.redcdn.hnyd.im.task.GetAllFriendAsyncTasks;
                    ...
                    switch (i) {
                        case RELATION_TYPE_POSITIVE:  //自己请求过加对方为好友
                            if (strangeMsgType == 0) { //  0|主动消息 (添加好友的时候发送的第一条消息)    1|回复请求   将好友关系更新未互为好友关系  添加到通讯录  并且告诉对端更新好友状态为已经是好友
                                ...
                                AuthenticateInfo mdsAccountInfo = AccountManager.getInstance(mContext).getmAuthInfo();
                                if (mdsAccountInfo!=null&&!"".equals(mdsAccountInfo)){
                                    String senderNubeNum = mdsAccountInfo.getNubeNumber();
                                    String receiverNubeNum = queryResultFriendInfo.getNubeNumber();
                                    String senderNickName = mdsAccountInfo.getNickname();
                                    String senderHeadurl = mdsAccountInfo.getHeadUrl();
