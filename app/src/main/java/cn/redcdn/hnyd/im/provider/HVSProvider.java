package cn.redcdn.hnyd.im.provider;
...
import cn.redcdn.hnyd.database.DBConstant;
import cn.redcdn.hnyd.database.DatabaseManager;
import cn.redcdn.hnyd.im.column.CollectionTable;
import cn.redcdn.hnyd.im.column.FriendRelationTable;
import cn.redcdn.hnyd.im.column.GroupMemberTable;
import cn.redcdn.hnyd.im.column.GroupTable;
import cn.redcdn.hnyd.im.column.NoticesTable;
import cn.redcdn.hnyd.im.column.NubeFriendColumn;
import cn.redcdn.hnyd.im.column.StrangerMessageTable;
import cn.redcdn.hnyd.im.column.ThreadsTable;
...
public class HVSProvider extends ContentProvider {
    ...
    static {
        ...
        uriMatcher.addURI(AUTHORITY, "t_friends_relation_order_by_update_time", T_FRIENDS_RELATION_ORDER_BY_UPDATE_TIME);
        ...
        uriMatcher.addURI(AUTHORITY, "stranger_message_table_insert", STRANGER_MESSAGE_TABLE_INSERT);
        uriMatcher.addURI(AUTHORITY, "stranger_message_table_delete", STRANGER_MESSAGE_TABLE_DELETE);
        uriMatcher.addURI(AUTHORITY, "stranger_message_table_update", STRANGER_MESSAGE_TABLE_UPDATE);
        uriMatcher.addURI(AUTHORITY, "stranger_message_table_query", STRANGER_MESSAGE_TABLE_QUERY);
        uriMatcher.addURI(AUTHORITY, "STRANGER_MESSAGE_TABLE_QUERY_NOT_READ_MSG", STRANGER_MESSAGE_TABLE_QUERY_NOT_READ_MSG);
    }
    ...
    public Cursor query(Uri uri, String[] projection, String selection,
        ...
        switch (uriMatcher.match(uri)) {
            ...
            case GET_APP_LINKMAN_DATA_NEW:
                // 获取应用联系人
                String selfNumberss = manager.getmAuthInfo().getNubeNumber();
            ...
            case NETPHONE_THREADS_GETALL:
                ...
                sql = "select th.id as threadsId,f.name,n2.nickName,n2.phoneNum,n2.nubeNumber"
                    ...
                    +
                    "where isDeleted=0) f on th.recipientIds = f.nubeNumber where th.top=0 order by th.top desc,th.lastTime desc";
                cursor = db.rawQuery(sql, null);
                break;
            case NETPHONE_THREADS_GETALL_TOP:
                ...
                sql = "select th.id as threadsId,f.name,n2.nickName,n2.phoneNum,n2.nubeNumber"
                        ...
                        + "tg on th.id = tg.gid left join (select * from t_nubefriend "
                        +
                        "where isDeleted=0) f on th.recipientIds = f.nubeNumber where th.top=1 order by th.reserverStr2 desc";
                cursor = db.rawQuery(sql, null);
                break;
            ...
            case T_FRIENDS_RELATION_ORDER_BY_UPDATE_TIME:
                ...
                sql = "select * from t_friends_relation inner join(select  strangerNubeNumber,max(time) maxTime from  t_stranger_msg group by strangerNubeNumber) c on  t_friends_relation.nubeNumber=c.strangerNubeNumber  where t_friends_relation.isDeleted=0 order by maxTime desc";
                cursor = db.rawQuery(sql, null);
                break;
            default:
                break;
