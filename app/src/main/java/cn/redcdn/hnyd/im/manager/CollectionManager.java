package cn.redcdn.hnyd.im.manager;
...
import cn.redcdn.hnyd.im.bean.CollectionBean;
import cn.redcdn.hnyd.im.bean.CollectionEntity;
...
import cn.redcdn.hnyd.im.collection.ButeleCollectionFile;
import cn.redcdn.hnyd.im.collection.CollectionFileManager;
import cn.redcdn.hnyd.im.collection.CollectionUpdateWorkManager;
import cn.redcdn.hnyd.im.dao.CollectionDao;
import cn.redcdn.hnyd.im.dao.GroupDao;
import cn.redcdn.hnyd.im.dao.NetPhoneDaoImpl;
import cn.redcdn.hnyd.im.fileTask.DownloadTaskManager;
...
import static cn.redcdn.hnyd.MedicalApplication.context;
...
public class CollectionManager{
    ...
    public void addCollectionByNoticesBean(Context context,NoticesBean bean){
        CustomLog.d("CollectionManager","addCollection body" + bean.getBody() + " uuid:" + bean.getId());
        //收藏中增加 ForwarderName、ForwarderHeaderUrl参数
        String newBody = bean.getBody();
        AuthenticateInfo loginUserInfo = AccountManager.getInstance(context).getmAuthInfo();
        if(loginUserInfo.getNubeNumber().equals(bean.getSender())){
            newBody = addNewMsgToBody(bean.getBody(),loginUserInfo.getNickname(),loginUserInfo.getHeadUrl(),loginUserInfo.getNubeNumber(),bean.getThreadsId());
        }else{
            ...
            if (friendBean != null && !TextUtils.isEmpty(friendBean.getNubeNumber())){
                newBody = addNewMsgToBody(bean.getBody(),friendBean.getNickname(),friendBean.getHeadUrl(),friendBean.getNubeNumber(),bean.getThreadsId());
            }else{
                ...
                if(groupDao.existGroup(bean.getThreadsId())){
                    GroupMemberBean memberBean = groupDao.queryGroupMember(bean.getThreadsId(),bean.getSender());
                    newBody = addNewMsgToBody(bean.getBody(),memberBean.getNickName(),memberBean.getHeadUrl(),bean.getSender(),bean.getThreadsId());
                }else {
                    if (bean.getSender().equals(SettingData.getInstance().adminNubeNum)){
                        String KEY_SERVICE_NUBE_INFO = "ServiceNubeInfo";
                        SharedPreferences preferences = MedicalApplication.getContext().getSharedPreferences(KEY_SERVICE_NUBE_INFO,context.MODE_PRIVATE);
                       String nickName = preferences.getString("USERNAME",SettingData.getInstance().adminNubeNum);
                        String headUrl = preferences.getString("HEAD_URL","");
                        newBody = addNewMsgToBody(bean.getBody(),nickName,headUrl,bean.getSender(),bean.getThreadsId());
                    }else{
                        newBody = addNewMsgToBody(bean.getBody(),bean.getSender(),"",bean.getSender(),bean.getThreadsId());
                    }
                }
        ...
        switch (code) {
            case -1:
                showToast(context,context.getString(R.string.collect_fail));
                break;
            case 1:
                showToast(context,context.getString(R.string.have_collected));
                break;
            case 0:
                showToast(context,context.getString(R.string.collect_success));
                break;
            default:
                break;
        }
    ...
    public void addCollectionByNoticesBeans(List<NoticesBean> beanList){
        if(beanList!=null&&beanList.size()>0){
            for(NoticesBean bean:beanList){
                ...
                AuthenticateInfo loginUserInfo = AccountManager.getInstance(context).getmAuthInfo();
                if(loginUserInfo.getNubeNumber().equals(bean.getSender())){
                    newBody = addNewMsgToBody(bean.getBody(),loginUserInfo.getNickname(),loginUserInfo.getHeadUrl(),loginUserInfo.getHeadUrl(),bean.getThreadsId());
                }else{
                    ContactFriendBean friendBean = new NetPhoneDaoImpl(context).queryFriendInfoByNube(bean.getSender());
                    if (friendBean != null && !TextUtils.isEmpty(friendBean.getNubeNumber())){
                        newBody = addNewMsgToBody(bean.getBody(),friendBean.getNickname(),friendBean.getHeadUrl(),friendBean.getNubeNumber(),bean.getThreadsId());
