...
import cn.redcdn.hnyd.im.task.AsyncTasks;
...
//        }
        ...
        if(!IMConstant.isP2PConnect){
            if(listener!=null){
                listener.onResult(interfaceName,false,mContext.getString(R.string.connecting_service_wait));
            }
    ...
    public void run(){
        ...
        if (!NetWorkUtil.isNetworkConnected(mContext)){
            listener.onResult(interfaceName,false,mContext.getString(R.string.net_connect_error_wait_try_again));
            showToast(mContext.getString(R.string.net_connect_error_wait_try_again));
            return;
        }
                            ...
                            new Thread(new Runnable() {
                                ...
                                public void run() {
                                    try {
                                        ...
                                        CustomLog.d(TAG,"GroupDetail doSaveorUpdateGroupDetail json解析出错");
                                        e.printStackTrace();
                                    }
                                    listener.onResult(interfaceName,true,mContext.getString(R.string.success));
                                }
...
//							TODO 目前未处理
                        ...
                        if ("-207".equals(resp.optString("status"))){
                            if (interfaceName.equals(UrlConstant.METHOD_QUITE_GROUP)){
                                listener.onResult(interfaceName,false,mContext.getString(R.string.cannot_hand_over));
                                showToast(mContext.getString(R.string.cannot_hand_over));
                            }else if (interfaceName.equals(UrlConstant.METHOD_CREATE_GROUP)) {
                                listener.onResult(interfaceName,false,mContext.getString(R.string.most_creat_group_chat_20));
                            ...
                            listener.onResult(interfaceName,false,mContext.getString(R.string.group_people_limit_100));
                            showToast(mContext.getString(R.string.group_people_limit_100));
                        }else if ("-205".equals(resp.optString("status"))){
                            if (interfaceName.equals(UrlConstant.METHOD_QUITE_GROUP)){
                                if (quitType==0){
                                    listener.onResult(interfaceName,false,mContext.getString(R.string.select_other_person));
                    ...
                    listener.onResult(interfaceName,false,mContext.getString(R.string.json_parsing_error));
                }
            }
        });
        invokeTask.setListenerFaliureResult(new AsyncTasks.ListenerFaliureResult() {
            ...
            public void getResluts(String msg,boolean alerted) {
                ...
                showToast(mContext.getString(R.string.interface_transfer_error));
                listener.onResult(interfaceName,false,mContext.getString(R.string.interface_transfer_error));
            }
        });
    ...
    public void resultParse(int reason, String reslutJson, final String interfaceName, String gid, String gname,
        ...
        if(reason!=0|| TextUtils.isEmpty(reslutJson)||TextUtils.isEmpty(interfaceName)){
            ...
            if (listener!=null){
                if(reason == -33){
                    CustomToast.show(mContext,mContext.getString(R.string.net_not_force_wait_try_again),CustomToast.LENGTH_SHORT);
                }else{
                    CustomToast.show(mContext,mContext.getString(R.string.interface_transfer_error),CustomToast.LENGTH_SHORT);
                }
                listener.onResult(interfaceName,false,mContext.getString(R.string.interface_transfer_error));
            }
            return;
...
//			mGroupDao.addGroupMember(members);
    ...
    private void insertdeleteMemberInfor(ArrayList<GroupMemberBean> members){
        ...
        String eventMsg=mContext.getString(R.string.you_will)+StringUtil.list2String(nameArrayList, '、')+mContext.getString(R.string.move_out_group);
        MessageGroupEventParse.insertGroupEventDiscription("",mGid, AccountManager.getInstance(mContext).getNube(),eventMsg, "");
    }
    ...
    private void insertInvitateMemberInfor(ArrayList<GroupMemberBean> members){
        ...
        String loginNube=AccountManager.getInstance(mContext).getmAuthInfo().getNubeNumber();
        ArrayList<String> nameArrayList=new ArrayList<String>();
        for (GroupMemberBean bean:members){
        ...
        String eventMsg=mContext.getString(R.string.you_invite)+ StringUtil.list2String(nameArrayList, '、')+mContext.getString(R.string.join_group);
        MessageGroupEventParse.insertGroupEventDiscription("",mGid,AccountManager.getInstance(mContext).getNube(), eventMsg, "");
    }
