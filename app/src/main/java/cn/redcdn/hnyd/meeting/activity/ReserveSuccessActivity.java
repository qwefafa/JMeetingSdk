package cn.redcdn.hnyd.meeting.activity;
...
import cn.redcdn.hnyd.meeting.util.DateUtil;
...
public class ReserveSuccessActivity extends BaseActivity implements View.OnClickListener {
    ...
    private void initData() {
        if (meetinfo != null) {
            ...
            if (meetinfo.getBookNube().equals(AccountManager.getInstance(this).getmAuthInfo().getNubeNumber())) {
    ...
    // 提供给IM使用   分享直播
     private void shareLiveByFriend(List<String> userList) {
         if (meetinfo != null) {
             ...
             String token = AccountManager.getInstance(this).getmAuthInfo().getAccessToken();
        ...
        switch (v.getId()) {
            ...
            case R.id.join_meeting_btn:
                ...
                if (hasPassword == 0) {//无会议密码
                    ...
                    if (String.valueOf(meetinfo.getBookNube()).equalsIgnoreCase(info.getNubeNumber())) {//我召开的
                        joinMeeting();
                    } else {//不是我召开的
                        ...
                        iv.putExtra("nube", info.getNubeNumber());
