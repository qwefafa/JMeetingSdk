...
//		}
	...
	private void joinMeeing(String meetingId) {
		...
		if (i == 0) {
			...
			cn.redcdn.hnyd.util.CustomToast.show(InputMeetingPasswordDialog.this, getString(R.string.join_consultation_fail),
				cn.redcdn.hnyd.util.CustomToast.LENGTH_SHORT);
		}
	}
	private void joinMeeting(String meetingId,String groupid) {
		...
		if (i == 0) {
			showLoadingView(getString(R.string.join_consultation), new OnCancelListener() {

				@Override
				public void onCancel(DialogInterface dialog) {
					dialog.dismiss();
					MedicalMeetingManage.getInstance().cancelJoinMeeting("HomeActivity");
					cn.redcdn.hnyd.util.CustomToast.show(getApplicationContext(), getString(R.string.cancel_get_consultation_info), Toast.LENGTH_SHORT);
				}
			});
		} else {
			cn.redcdn.hnyd.util.CustomToast.show(InputMeetingPasswordDialog.this, getString(R.string.join_consultation_fail),
