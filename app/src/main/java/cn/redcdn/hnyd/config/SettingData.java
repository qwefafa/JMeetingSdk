...
public class SettingData {
    ...
    public static String VIRTUAL_DEVICE_NUM = "GDD411111111145"; //虚拟串号，用于nps获取
//    public static String VIRTUAL_DEVICE_NUM = "GDD811111111145"; //虚拟串号，用于nps获取
    public static String NpsDeviceType = "MobileHnyd"; //设备类型，用于hvs医疗云平台获取nps地址
    ...
    //nps地址
    public String NPS_URL = "http://175.102.132.82:8018/nps_x1/";
    //"http://175.102.132.82:8018/nps_x1/";//"http://xmeeting.butel.com/nps_x1/";
    //双服务器第二个nps地址
    public String SLAVE_NPS_URL = "http://175.102.132.82:8018/nps_x1/";
    //http://103.25.23.83:8018/nps_x1
    ////    nps地址
...
//        public String SLAVE_NPS_URL = "http://xmeeting.butel.com/nps_x1/";
    ...
    public String readNpsUrlFromLocal() {
        File npsfile = new File(
            Environment.getExternalStorageDirectory().getPath() + "/" + LogRootDir
    ...
    private SettingData() {
        ...
        rootPath = MedicalApplication.shareInstance()
            .getDir(MedicalApplication.context.getPackageName(), Context.MODE_PRIVATE)
            .getAbsolutePath();
...
//                    SettingData.getInstance().ENTERPRISE_CENTER_URL = commonObj
//                            .getString(NpsParamConfig.Medical_COMMON_PUC_URL);
                    SettingData.getInstance().ENTERPRISE_CENTER_URL = "http://211.103.143.72:81/";
//TODO nps暂时没有配置个人通讯录服务器，需要注释掉解析
...
//                        .getString(NpsParamConfig.Medical_COMMON_Favorite_Server_Url);
                    ...
                    SettingData.getInstance().MDS_URL
                        = "http://testmedical.butel.com/mds/externalservice/";
                    SettingData.getInstance().Slave_MDS_URL = "";
                    ...
                    SettingData.getInstance().GROUP_MANAGER_URL = commonObj.getString(
                        NpsParamConfig.Medical_COMMON_GROUP);
//TODO
//                    SettingData.getInstance().Friend_Url = commonObj.getString(
//                        NpsParamConfig.COMMON_Friendship_Service_URL);
                    if (commonObj.has(NpsParamConfig.COMMON_ArticlePreview)) {
                        SettingData.getInstance().Article_Preview_Url = commonObj.getString(
                            NpsParamConfig.COMMON_ArticlePreview);
                    }
                }
                ...
                // TODO 升级地址nps配置
                SettingData.getInstance().AppUpdateConfig.Master_ServerUrl
                    = "175.102.132.82:10002/";
                String slave_serverUrl = "103.25.23.83:10002/";

                if (!slave_serverUrl.equals("")) {
                    SettingData.getInstance().AppUpdateConfig.Slave_ServerUrl = slave_serverUrl;
                }

                SettingData.getInstance().AppUpdateConfig.ProjectName = "justmeetinghvs";
                SettingData.getInstance().AppUpdateConfig.DeviceType = "MOBILE";

                SettingData.getInstance().LogUploadConfig.ServerIP = "103.25.23.83";
                SettingData.getInstance().LogUploadConfig.ServerPort = 10101;

                // 获取帮助参数
                SettingData.getInstance().REVIEW_NUM = "400-620-0353";
                SettingData.getInstance().SERVICEAGREEMENT_URL
                    = "http://testmedical.butel.com/mws/index.html";
                SettingData.getInstance().DOWNLAOD_LINK
                    = "http://testmedical.butel.com/mws/qrcodeForward.html?id=";

                SettingData.getInstance().CUSTEMER_TEL1 = "90000082";
                SettingData.getInstance().CUSTEMER_TEL2 = "22222222";

                SettingData.getInstance().ShareUrl
                    = "http://testmedical.butel.com/mws/qrcodeForward.html?v=621";

                SettingData.getInstance().adminNubeNum = "66910000";

                SettingData.getInstance().adminHeadUrl = "";

                SettingData.getInstance().adminNickName = "adminNickName";
    ...
    public void restoreNpsConfigToSharePre(String npsString) {
        ...
        SharedPreferences sharedPreferences = MedicalApplication.shareInstance()
            .getSharedPreferences("CACHE_NPS_CONFIG", Activity.MODE_PRIVATE);
    ...
    public String readNpsConfigFromSharePre() {
        CustomLog.d(TAG,
            "SettingData::readHttpRequestConfigFromSharePre 从SharePreference 中读取 nps缓存数据");
    ...
    private void LogHttpRequestConfig() {
        ...
        CustomLog.i(TAG, "ConstConfig.enterPriseUserCenterWebDomain：" +
        ...
        CustomLog.i(TAG, "ConstConfig.masterAppUpdateServerWebDomain：" +
            ConstConfig.masterAppUpdateServerWebDomain);
        CustomLog.i(TAG, "ConstConfig.slaveAppUpdateServerWebDomain：" +
            ConstConfig.slaveAppUpdateServerWebDomain);
